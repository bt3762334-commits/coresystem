...
const Q = [
  { key: "q1", badge: "Q1", label: "افعلها الآن",   title: "عاجل ومهم",         icon: "🔥", c: "#EF4444", lt: "#FEE2E2", dk: "#991B1B", hint: "ركّز عليها فوراً" },
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{
        ...
        <div style={{ flex: 1, minWidth: 120, height: 6, background: "var(--border)", borderRadius: 3, overflow: "hidden" }}>
          <div style={{
      ...
      {total > 0 && (
        <div style={{ display: "grid", gridTemplateColumns: "32px 1fr 1fr", gridTemplateRows: "32px 1fr 1fr", gap: 0 }} className="matrix-grid">
          <div style={{
...
function Quadrant({ q, tasks, input, onInput, onAdd, onDone, onDelete }) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        background: `linear-gradient(90deg, ${q.c}, ${q.c}AA)`,
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 7, flexWrap: "wrap", marginTop: 4 }}>
        ...
        <span style={{ fontSize: 11, color: "var(--text-2)" }}>{done}/{tasks.length}</span>
      </div>
      <p style={{ fontSize: 11, color: "var(--text-2)", fontStyle: "italic", margin: 0 }}>{q.hint}</p>
      <div style={{
        flex: 1, overflowY: "auto",
        display: "flex", flexDirection: "column", gap: 5, maxHeight: 170,
      }}>
        {tasks.length === 0 && (
        ...
        {tasks.map((t) => (
          <div key={t.id} style={{
            ...
            <button onClick={() => onDelete(t.id)} style={{
              fontSize: 15, color: "var(--text-2)", background: "none",
      ...
      <div style={{ display: "flex", gap: 6 }}>
        ...
        <button
          ...
          style={{
            ...
            border: "none", background: q.lt, color: q.dk,
